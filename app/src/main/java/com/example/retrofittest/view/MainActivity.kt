...
class MainActivity : AppCompatActivity() {
    ...
    private lateinit var mainViewModel: MainViewModel
     ...
     var tstReqBody= InitUploadRequest("movie/quicktime", "test.mov")
...
//        initObservers()
    ...
    private fun initObservers() {
        mainViewModel.isLoading.observe(this) { isLoading ->
            if (isLoading) activityMainBinding.weatherTv.text =
                resources.getString(R.string.loading)
        }
        mainViewModel.isError.observe(this) { isError ->
            if (isError) activityMainBinding.weatherTv.text = mainViewModel.errorMessage
    ...
    private fun fetch() {
        //get data
        mainViewModel.fetchData(tstReqBody)
    }
}